...
Mesh* MeshBuilder::GenerateSpriteSheet(const std::string &meshName, unsigned numRow, unsigned numCol)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, std::vector<unsigned char> heightMap)
{
	...
	//vector<vector<float>> heightTest(terrainSize, vector<float>(terrainSize));
	...
	//heightsInfo = heightTest;

	for(unsigned z = 0; z < terrainSize - 1; ++z) 
	{  
		for(unsigned x = 0; x < terrainSize - 1; ++x)  
		{    
			index_buffer_data.push_back(terrainSize * z + x);  //Tri 1   
...
Mesh* MeshBuilder::GenerateLineOfSight(const std::string &meshName, Color color, float length, float tileSize)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	if (length <= 3)
	{
		...
		v.pos.Set(length * tileSize - length * (tileSize * 0.4f), length * tileSize + (tileSize * 0.5f), 0.f);
		v.color = color;
		vertex_buffer_data.push_back(v);
	...
	else
	{
		...
		v.pos.Set(3.f * tileSize - 3.f * (tileSize * 0.4f), length * tileSize + (tileSize * 0.5f), 0.f);
		v.color = color;
		vertex_buffer_data.push_back(v);
